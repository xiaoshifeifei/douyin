  ...
  <div id="jurisdiction" class="LoadingArea">
    <el-card>
      ...
      <el-dialog :title="edatPa" :visible.sync="dialoInfo" width="800">
        <div class="formStyle">
          ...
          >
            ...
            >
              <el-input
                v-model="Info.business_license_url"
                :disabled="edatPa == '详情'"
              ></el-input>
            </el-form-item>
            <div style="margin-top:20px"></div>
            ...
            <el-form-item>
              <el-button class="dialo" @click="dialoInfo = false"
                >取消</el-button
              >
              <el-button
                type="primary"
      ...
      >
        ...
        >
          <el-form-item label="营业执照" class="bgctable">
            <el-input
          ...
          <el-form-item label="门店logo" class="bgctable ">
            <el-input v-model="Info.avatar" :disabled="edatPa=='详情'"></el-input>
          </el-form-item>
          <el-form-item label="抖音号" class="bgctable ">
            <el-input v-model="Info.tiktok_num" :disabled="edatPa=='详情'"></el-input>
          </el-form-item>
          <el-form-item
            label="是否蓝V"
            v-if="Info.check_status == '2'"
            class="bgctable"
          >
            <el-input v-model="Info.tiktok_num" :disabled="edatPa=='详情'"></el-input>
          </el-form-item>
          <el-form-item label="门店名称" class="bgctable ">
            <el-input v-model="Info.store_name" :disabled="edatPa=='详情'"></el-input>
          </el-form-item>
          <el-form-item label="联系人" class="bgctable ">
            <el-input v-model="Info.contacts" :disabled="edatPa=='详情'"></el-input>
          </el-form-item>
          <el-form-item label="联系人手机号" class="bgctable ">
            <el-input v-model="Info.tel" :disabled="edatPa=='详情'"></el-input>
          </el-form-item>
          <el-form-item label="所在省市县" class="bgctable ">
            <el-input
              v-model="Info.province_city_district_name"
              :disabled="edatPa=='详情'"
            ></el-input>
          </el-form-item>
          <el-form-item label="详细地址" class="bgctable ">
            <el-input v-model="Info.address" :disabled="edatPa=='详情'"></el-input>
          </el-form-item>
          <el-form-item label="门店简介" class="bgctable">
            <el-input
        ...
        >
          <div class="divbox">
            ...
            <div class="box-r">
              ...
              <div class="box2">
                {{
                  this.Info.check_status == "-1"
                    ? "已驳回"
                    : this.Info.check_status == "0"
                    ? "待提交"
                    : this.Info.check_status == "1"
                    ? "待审核"
                    : this.Info.check_status == "2"
                    ? "已通过"
                    : ""
        ...
        <div class="video1" v-if="edatPa == '申请成功'">
          <el-button
            class="btn1"
            v-if="myParams == ''"
            @click="rejectClick"
          ...
          <el-button
            ...
            >通过</el-button
          >
          <el-button v-else class="btn3" @click="isShowClick">关闭</el-button>
          <div style="clear:both"></div>
        </div>
        <div class="video4" v-if="edatPa == '详情'">
          <el-button class="btn1" @click="isShowClick">关闭</el-button>
        </div>
        <div class="video4" v-if="edatPa == '编辑'">
          <el-button class="btn2" @click="isShowClick">关闭</el-button>
          <el-button type="primary" @click="isShowClick">确认</el-button>
        </div>
      </el-dialog>
      <!-- 驳回 -->
      <el-dialog
        title="驳回"
        :before-close="isRejectClick"
      ...
      >
        ...
        >
          <el-form-item label="驳回理由">
            <el-input v-model="rejectInfo" type="textarea" rows="5"></el-input>
          </el-form-item>
        </el-form>

        <div class="video3">
          <el-button class="btn1" @click="isRejectClick">取消</el-button>

          <el-button class="btn2" type="primary" @click="commitReject"
            >确认</el-button
          >
          <div style="clear:both"></div>
        </div>
      ...
      >
        ...
        >
          <el-form-item label="是否蓝V">
            <el-switch v-model="value11"> </el-switch>
          </el-form-item>
          <el-form-item label="抖音号分享地址">
            <el-input v-model="dyInfo"></el-input>
          </el-form-item>
        </el-form>

        <div class="video2">
          <el-button class="btn1" @click="isGoClick">取消</el-button>
          <el-button class="btn2" type="primary" @click="commitGo"
            >确认</el-button
          >
          <div style="clear:both"></div>
        </div>
      ...
      >
        ...
        >
          ...
          <el-table-column prop="update_time" align="center" label="提交时间" />
...
export default {
  data() {
    return {
      ...
      roleRuless: {
        ...
        value11: [{ required: true, message: "请选择是否蓝V" }],
        dyInfo: [{ required: true, message: "请填写抖音号分享地址" }]
      },
      roles: {
        value11: [{ required: true, message: "请选择是否蓝V" }],
        dyInfo: [{ required: true, message: "请填写抖音号分享地址" }]
  ...
  methods: {
    ...
    // 审核通过确认
    async commitGo() {
      console.log("this.Tnfo", this.Info.dyInfo);
      console.log(this.value11);
      let res = await storeAdopts({
        store_id: this.Info.store_id,
        share_url: this.dyInfo,
        check_remark: "通过",
        is_bluev: this.value11 == true ? "1" : "0"
      });
      if (res.code == 0) {
    ...
    async commitReject() {
      ...
      let res = await storeReject({
        store_id: this.myId,
        check_remark: this.rejectInfo
      });
      if (res.code == 0) {
    ...
    async handleBatchDownload(selectImgList) {
      ...
      Promise.all(promises).then(() => {
        zip
          ...
          .then(content => {
            // 生成二进制流
            FileSaver.saveAs(content, "门店抖音号审核数据.zip"); // 利用file-saver保存文件
          });
      });
      console.log("成功啦");
    },
    ...
    exportExcel() {
      ...
      let tittle =
        thirteenBitTimestamp(new Date().getTime()) + "门店抖音号审核数据.xlsx";
      try {
        FileSaver.saveAs(
          new Blob([wbout], {
    ...
    async commitClick() {
      const data = {
        share_url: share_url,
        is_bluev: is_bluev,
        check_remark: this.Info.remark,
        store_id: this.myId
      };
      try {
        let res = await storeAdopts(data);
        if (res.code === 0) {
    ...
    async getList() {
      ...
      let res = await storeLists({
        keywords: this.keywords,
        page: this.page,
        limit: this.limit,
        check_status: this.value1,
    ...
    async handleInfo(roleData) {
      ...
      let res = await storeRead(roleData.store_id);
      console.log("res", res);
      if (res.code == 0) {
