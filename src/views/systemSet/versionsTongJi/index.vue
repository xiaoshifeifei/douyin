...
<template>
  <el-card>
      ...
      >
        ...
        <el-table-column label="操作" align="center" fixed="right">
          <template slot-scope="scope">
            <el-button
              type="primary"
              @click="handleEditApp(scope.row.id)"
              size="mini"
              >查看</el-button
            >
            <el-button
            ...
            <!-- <el-link type="primary" @click="handleEditApp(scope.row.id)">
    ...
    <el-pagination
      ...
      :current-page="app.tableData.pageNo"
      :page-size="app.tableData.pageSize"
    ...
    >
      <el-form
        ...
        label-position="left"
      >
        <el-form-item label="创建版本号" prop="versionno">
          <el-input v-model="app.detail.versionno"></el-input>
        </el-form-item>

        <el-form-item label="版本类型" prop="versionnotype">
          <el-select v-model="app.detail.versionnotype" placeholder="请选择">
            <el-option label="android" :value="0"></el-option>
            <el-option label="IOS" :value="1"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="是否强制更新" prop="mustupdate">
          <el-select v-model="app.detail.mustupdate" placeholder="请选择">
            <el-option label="否" :value="0"></el-option>
            <el-option label="是" :value="1"></el-option>
          </el-select>
        </el-form-item>
        <!-- action="http://bytesapi.hizhiliao.cn/backend/version/upload" -->
        <el-form-item label="APP安装包">
          <el-upload
            ...
            accept=".jpg,.png,.apk,.mp4,.mov,.avi,.exe"
          >
            <el-button v-loading="app.loading" size="small" type="primary"
              >点击上传</el-button
            >
          </el-upload>
        </el-form-item>
        <el-form-item label="网络URL地址" prop="cdnurl">
          <el-input v-model="app.detail.cdnurl"></el-input>
        </el-form-item>
        <el-form-item label="版本说明" prop="upgradedesc">
          <el-input
            type="textarea"
            :rows="3"
            v-model="app.detail.jilu"
          ></el-input>
        </el-form-item>
        <el-form-item label="版本记录" prop="upgradedesc">
          <el-input
            type="textarea"
            :rows="3"
            v-model="app.detail.upgradedesc"
          ></el-input>
        </el-form-item>
      </el-form>
      <span slot="footer">
...
export default {
  data() {
    return {
      ...
      actionUrl: process.env.VUE_APP_BASE_API + "/version/upload",
      columns: [
        { prop: "version", label: "版本号", width: 100 },
        { prop: "remark", label: "版本记录", width: 100 },
        { prop: "is_force", label: "强制升级", width: 100 },
        { prop: "memo", label: "版本说明", width: 100 },
        { prop: "url", label: "网络URL", width: 100 }
      ],
      app: {
        tableData: {
          list: [],
  ...
  methods: {
    ...
    async handleEditApp(id) {
      ...
      try {
        let res = await reqVersionDetail(id);
    ...
    onSaveDetail() {
      this.$refs.detail.validate(async valid => {
        ...
        let data = {
          ...
          remark: this.app.detail.jilu,
          memo: this.app.detail.upgradedesc
        };
        if (!this.isCreate) {
