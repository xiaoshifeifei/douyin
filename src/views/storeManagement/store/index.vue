...
<template>
  <div>
    ...
    >
      ...
      >
        ...
        <el-form-item label="创建时间" prop="create_time">
          <el-input v-model="app.detail.create_time" v-if="app.detail.create_time" :disabled="isCreate == '查看'"></el-input>
        </el-form-item>
    ...
    >
      ...
      <span slot="footer">
        <el-button @click="resetDetailss">取消</el-button>
        <el-button
          v-loading="app.saveLoading"
          type="primary"
          @click="onSaveDetails"
  ...
  data() {
    return {
      ...
      columns: [
        ...
        // { prop: "check_status", label: "审核状态", width: 60 },
        { prop: "check_status_text", label: "审核状态", width: 100 }
      ],
      app: {
        tableData: {
          list: [],
  ...
  methods: {
    ...
    async onSaveDetails() {
      if (this.value2.trim() == '' ) {
       ...
       const data = {
          store_id:this.paramsObj.store_id,
          days:this.value2
        }
    ...
    async handleEditApp(id) {
      ...
      try {
        ...
        this.app.detail.create_time = res.data.create_time;  //添加时间
